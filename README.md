...
一个用于演示的简单投票系统，基于 Paillier 同态加密算法实现。
...
系统主要分为以下三个核心组件：
1. **投票加密**：选民在本地设备上对选票进行加密，然后再发送给服务器。
2. **同态聚合**：服务器利用同态加法特性（即密文相乘），将所有加密的选票聚合在一起。
3. **解密公布**：使用私钥对最终聚合后的密文进行解密，得出最终的选举结果。

## 功能特点

- **Paillier 实现**：从零开始实现了大整数运算、素数生成（Miller-Rabin 素性检验）以及模算术等底层密码学算法。
- **本地加密**：选票在离开选民设备前就已经被完全加密。
